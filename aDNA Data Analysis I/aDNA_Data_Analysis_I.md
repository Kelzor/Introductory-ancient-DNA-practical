...
Open the fragment misincorporation plots for each sample. The four plots at the top indicate base (A, C, T, and G) frequencies within the read (within the grey square) and of the reference genome outside of the read. What are these plots illustrating? *Hint: depurination.*  

The bottom two plots are the most common damage plots that you may have seen before. The red line charts **C to T** substitutions, and the blue line charts **G to A** substitutions. Remeber these base changes in DNA fragments result from hydrolysis (water damage) and deamination (loss of an amine group). They are common **miscoding lesions** in aDNA.
...
- From the bottom two plots, estimate the values `3 Prime G>A 1st base (%)` and `5 Prime C>T 1st base (%)` in your mtDNA mapping report.
...
**Variant calling** is the process of identifying genetic variants in your sample based on the sequence alignments. Point mutation **SNPs/SNVs** are the most common variants in aDNA. This is because it is hard to confidently identify structural variants due to incomplete genome recovery.
...
Instead of using the `.vcf` files to identify variants in our samples, we are going to manually investigate the `.bam` files in **Integrated Genome Browser (IGV)**. Our genomic data are low coverage, so we need to manually inspect SNPs. We will also use this as an opportunity to see how different decontamination methods modified our `.bam` files.
...
- You should bee looking at the `.bam`, `.rescale.bam`, and `.pmds3filter.bam` files for one sample at a time, e.g. all three bams for UF801.

- The three bams for the same library give you different information to help you assess SNP/SNV authenticity. `.bam` is the quality filtered and dedupped bam. The `.rescale.bam` was generated by MapDamage and has deviations from the reference that are likely to be miscoding lesions from damage grayed out to indicate low confidence. `.pmds3filter.bam` was generated by the tool [**PMDtools**](https://github.com/pontussk/PMDtools). Whereas MapDamage rescales quality scores of likely damaged bases, PMDtools removes all reads from a .bam file that do not have signals of damage in the terminal bases.   
...
For each sample, manually scan the hypervariable regions using the arrows at the top. It will be easier for you to zoom in all the way using the **+** in the top right corner. Once you zoom in all the way, you can move through the genome 45 bp at a time using the small black arrow on the right side of the screen.
...
- While you are scanning the sites, compare the differences in the three `.bam` files. Remember, `.bam` hasn’t been filtered for contamination, `.rescaled.bam` has had its damaged bases rescaled to low qualities, and `.pmds3filter.bam` has been filtered to remove reads without terminal bases, i.e., contaminant reads.
...
You can view the automated mapping and summary report generated by nf-core/eager for the practical data here [Practical-mtDNA-multiqc_report.html](https://kelzor.github.io/Introductory-ancient-DNA-practical/aDNA%20Data%20Analysis%20I/Practical-mtDNA-multiqc_report.html). 

If you've made it this far, pat yourself on the back! You've done your first analysis of genomic aDNA data from a high-impact paper.
